...
from src.data.labels import DIAGNOSIS_LABELS  # libellé FR d'un code SQL, pas du mock.
...
def render_correlation_matrix(line_id: str | None = None) -> None:
    ...
    with loading_wrapper("Chargement Correlation matrix…", "⏳"):
        try:
        ...
        detail_df = pd.DataFrame(
            [
                {
                    # (audit Pro TCL B3) : libellé lisible L66 au lieu
                    # de l'identifiant SYTRAL brut (ActIV:Line::66:SYTRAL_h20).
                    "Ligne": clean_line_label(s["line_id"]),
                    "Segment": s["name"],
                    # (audit Pro TCL B-14) : libellés FR pour rester
