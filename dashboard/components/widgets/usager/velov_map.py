...
def _load_stations_with_predictions() -> pd.DataFrame:
    ...
    # Prédiction H+1h uniquement (peut être vide si DAG pas tourné)
    pred_h1 = cached_velov_predictions(horizon_minutes=60)

    if not pred_h1.empty and "station_id" in pred_h1.columns:
        pred_h1_latest = (
...
def render_velov_map_compact(*, height: int = 280, key_suffix: str = "") -> None:
    ...
    try:
        ...
        st.dataframe(
            df[["station_name", "bikes_available", "predicted_bikes_h1"]].head(20),
