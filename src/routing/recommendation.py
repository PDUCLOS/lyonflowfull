...
def _build_bus_alternative(
    ...
    walk_to_stop = max(1, round(dist_stop / 1000.0 / 4.5 * 60.0))
    ...
    bus_dur = max(1, round(dist_bus / 1000.0 / 18.0 * 60.0))
...
def _build_vtc_alternative(
    ...
    dist = _haversine_m(origin_lat, origin_lon, dest_lat, dest_lon)
    direct_dur = max(1, round(dist / 1000.0 / 30.0 * 60.0))
